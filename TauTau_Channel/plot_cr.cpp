...
int main(){
 ...
 // DY Control Region parameters
 double min_aco_DY=0.0, max_aco_DY=0.25; int bin_aco_DY=5;
 double min_m_DY=70, max_m_DY=100; int bin_m_DY=5;
 double min_r_DY=-2, max_r_DY=2; int bin_r_DY=5;
 double min_pt_DY=200, max_pt_DY=350; int bin_pt_DY=10;

 // QCD Control Region parameters  
 double min_aco_QCD=0.8, max_aco_QCD=1.0; int bin_aco_QCD=6;
 double min_m_QCD=0, max_m_QCD=500; int bin_m_QCD=5;  // Fixed to match cuts!
 double min_r_QCD=-2, max_r_QCD=2; int bin_r_QCD=10;
 double min_pt_QCD=0, max_pt_QCD=80; int bin_pt_QCD=5;

 // TT Control Region parameters
 double min_aco_TT=0.5, max_aco_TT=1.0; int bin_aco_TT=8;
 double min_m_TT=200, max_m_TT=650; int bin_m_TT=6;  // Fixed to match cuts!
 double min_r_TT=-2, max_r_TT=2; int bin_r_TT=10;
 double min_pt_TT=0, max_pt_TT=300; int bin_pt_TT=5;  // Fixed to match cuts!
 ...
 TH1D h_pt_ttjets_TT("h_pt_ttjets_TT","",bin_pt_TT, min_pt_TT, max_pt_TT);
    ...
    for(int i=0; i<n_evt_data; i++){
        ...
        if(tree_data->GetLeaf("sist_mass")->GetValue(0) >= 0){
            ...
            // double tau_mass=13000.*sqrt(tree_data->GetLeaf("xi_arm1_1")->GetValue(0)*tree_data->GetLeaf("xi_arm2_1")->GetValue(0));
            cout<<"tau mass data"<<tau_mass<<endl;
            double tau_pt=tree_data->GetLeaf("tau0_pt")->GetValue(0);
            //cout<<"pt data"<<sist_pt<<endl;


            bool isDY_CR = (tau_mass >= 40 && tau_mass <= 100) && (sist_acop < 0.3) && (n_bjets == 0);
            bool isQCD_CR = (tau_mass >= 100 && tau_mass <= 300) && (sist_acop > 0.8) && (n_bjets == 0) ;
            bool isTT_CR = (tau_mass >= 200 && tau_mass <= 650) && (sist_acop > 0.5) && (n_bjets >= 1) && (tau_pt < 125);

            // Define control region cuts ONCE per event
            // bool isDY_CR = (sist_mass >= 40 && sist_mass <= 100) && (sist_acop < 0.3) && (n_bjets == 0);
            // bool isQCD_CR = (sist_mass >= 100 && sist_mass <= 300) && (sist_acop > 0.8) && (n_bjets == 0) && (sist_pt < 75);
            // bool isTT_CR = (sist_mass >= 200 && sist_mass <= 650) && (sist_acop > 0.5) && (n_bjets >= 1) && (sist_pt < 125);
            
            // Now fill ALL variables for ALL control regions using the pre-defined variables
            if(isDY_CR) {
                h_aco_data_DY.Fill(sist_acop);
                h_m_data_DY.Fill(sist_mass);
                h_r_data_DY.Fill(sist_rap);
                h_pt_data_DY.Fill(sist_pt);
            }
            if(isQCD_CR) {
                h_aco_data_QCD.Fill(sist_acop);
                h_m_data_QCD.Fill(sist_mass);
                h_r_data_QCD.Fill(sist_rap);
                h_pt_data_QCD.Fill(sist_pt);
            }
            if(isTT_CR) {
                h_aco_data_TT.Fill(sist_acop);
                h_m_data_TT.Fill(sist_mass);
                h_r_data_TT.Fill(sist_rap);
                h_pt_data_TT.Fill(sist_pt);
            }
        }
    }
    ...
    for(int i=0; i<n_evt_dy; i++){
        ...
        if(tree_dy->GetLeaf("sist_mass")->GetValue(0) >= 0){
            ...
            double tau_mass=sqrt(2*tau0_pt*tau1_pt*(cosh_delta_eta - cos_delta_phi));
            cout<<"tau mass data"<<tau_mass<<endl;
            double tau_pt=tree_dy->GetLeaf("tau0_pt")->GetValue(0);

            // Define control region cuts ONCE per event

            bool isDY_CR = (tau_mass >= 40 && tau_mass <= 100) && (sist_acop < 0.3) && (n_bjets == 0);
            bool isQCD_CR = (tau_mass >= 100 && tau_mass <= 300) && (sist_acop > 0.8) && (n_bjets == 0) && (tau_pt < 75);
            bool isTT_CR = (tau_mass >= 200 && tau_mass <= 650) && (sist_acop > 0.5) && (n_bjets >= 1) && (tau_pt < 125);
            
            // Now fill ALL variables for ALL control regions using the pre-defined variables
            if(isDY_CR) {
                h_aco_dy_DY.Fill(sist_acop);
                h_m_dy_DY.Fill(sist_mass);
                h_r_dy_DY.Fill(sist_rap);
                h_pt_dy_DY.Fill(sist_pt);
            }
            if(isQCD_CR) {
                h_aco_dy_QCD.Fill(sist_acop);
                h_m_dy_QCD.Fill(sist_mass);
                h_r_dy_QCD.Fill(sist_rap);
                h_pt_dy_QCD.Fill(sist_pt);
            }
            if(isTT_CR) {
                h_aco_dy_TT.Fill(sist_acop);
                h_m_dy_TT.Fill(sist_mass);
                h_r_dy_TT.Fill(sist_rap);
                h_pt_dy_TT.Fill(sist_pt);
            }
        }
    }
    ...
    for(int i=0; i<n_evt_qcd; i++){
        ...
        if(tree_qcd->GetLeaf("sist_mass")->GetValue(0) >= 0){
            ...
            double tau_mass=sqrt(2*tau0_pt*tau1_pt*(cosh_delta_eta - cos_delta_phi));
            cout<<"tau mass data"<<tau_mass<<endl;
            double tau_pt=tree_qcd->GetLeaf("tau0_pt")->GetValue(0);
            ...
            // Now fill ALL variables for ALL control regions using the pre-defined variables
            if(isDY_CR) {
                h_aco_qcd_DY.Fill(sist_acop);
                h_m_qcd_DY.Fill(sist_mass);
                h_r_qcd_DY.Fill(sist_rap);
                h_pt_qcd_DY.Fill(sist_pt);
            }
            if(isQCD_CR) {
                h_aco_qcd_QCD.Fill(sist_acop);
                h_m_qcd_QCD.Fill(sist_mass);
                h_r_qcd_QCD.Fill(sist_rap);
                h_pt_qcd_QCD.Fill(sist_pt);
            }
            if(isTT_CR) {
                h_aco_qcd_TT.Fill(sist_acop);
                h_m_qcd_TT.Fill(sist_mass);
                h_r_qcd_TT.Fill(sist_rap);
                h_pt_qcd_TT.Fill(sist_pt);
            }
        }
    }
    ...
    for(int i=0; i<n_evt_ttjets; i++){
        ...
        if(tree_ttjets->GetLeaf("sist_mass")->GetValue(0) >= 0){
            ...
            double tau_mass=sqrt(2*tau0_pt*tau1_pt*(cosh_delta_eta - cos_delta_phi));
            cout<<"tau mass data"<<tau_mass<<endl;
            double tau_pt=tree_ttjets->GetLeaf("tau0_pt")->GetValue(0);
            ...
            // Now fill ALL variables for ALL control regions using the pre-defined variables
            if(isDY_CR) {
                h_aco_ttjets_DY.Fill(sist_acop);
                h_m_ttjets_DY.Fill(sist_mass);
                h_r_ttjets_DY.Fill(sist_rap);
                h_pt_ttjets_DY.Fill(sist_pt);
            }
            if(isQCD_CR) {
                h_aco_ttjets_QCD.Fill(sist_acop);
                h_m_ttjets_QCD.Fill(sist_mass);
                h_r_ttjets_QCD.Fill(sist_rap);
                h_pt_ttjets_QCD.Fill(sist_pt);
            }
            if(isTT_CR) {
                h_aco_ttjets_TT.Fill(sist_acop);
                h_m_ttjets_TT.Fill(sist_mass);
                h_r_ttjets_TT.Fill(sist_rap);
                h_pt_ttjets_TT.Fill(sist_pt);
            }
        }
    }
...
cout << "DY CR - ttjets MC events: " << h_aco_ttjets_DY.GetEntries() << endl;
    ...
    TLine line_aco(min_aco_DY, 1.0, max_aco_DY, 1.0);  // Use DY ranges!
    ...
    TLine line_m(min_m_DY, 1.0, max_m_DY, 1.0);  // Use DY ranges!
    ...
    TLine line_r(min_r_DY, 1.0, max_r_DY, 1.0);  // Use DY ranges!
    ...
    TLine line_pt(min_pt_DY, 1.0, max_pt_DY, 1.0);  // Use DY ranges!
    ...
    leg5->SetFillStyle(0);
    leg5->AddEntry(&h_aco_data_QCD, "Data (2018)", "lep");
    leg5->AddEntry(&h_aco_qcd_QCD, "QCD (Data driven)", "f");
    leg5->AddEntry(&h_aco_dy_QCD, "Drell Yan", "f");
    leg5->AddEntry(&h_aco_ttjets_QCD, "t#bar{t}", "f");
    leg5->Draw();
    ...
    TLine line_aco_qcd(min_aco_QCD, 1.0, max_aco_QCD, 1.0);  // Use QCD ranges!
    line_aco_qcd.SetLineStyle(2);
    line_aco_qcd.SetLineColor(kRed);
    line_aco_qcd.Draw();
    
    c5.SaveAs("output_plots/QCD_CR_acoplanarity.png");
    ...
    leg6->SetFillStyle(0);
    leg6->AddEntry(&h_m_data_QCD, "Data (2018)", "lep");
    leg6->AddEntry(&h_m_qcd_QCD, "QCD (Data driven)", "f");
    leg6->AddEntry(&h_m_dy_QCD, "Drell Yan", "f");
    leg6->AddEntry(&h_m_ttjets_QCD, "t#bar{t}", "f");
    leg6->Draw();
    ...
    TLine line_m_qcd(min_m_QCD, 1.0, max_m_QCD, 1.0);  // Use QCD ranges!
    line_m_qcd.SetLineStyle(2);
    line_m_qcd.SetLineColor(kRed);
    line_m_qcd.Draw();
    
    c6.SaveAs("output_plots/QCD_CR_mass.png");
    ...
    leg7->SetFillStyle(0);
    leg7->AddEntry(&h_r_data_QCD, "Data (2018)", "lep");
    leg7->AddEntry(&h_r_qcd_QCD, "QCD (Data driven)", "f");
    leg7->AddEntry(&h_r_dy_QCD, "Drell Yan", "f");
    leg7->AddEntry(&h_r_ttjets_QCD, "t#bar{t}", "f");
    leg7->Draw();
    ...
    TLine line_r_qcd(min_r_QCD, 1.0, max_r_QCD, 1.0);  // Use QCD ranges!
    line_r_qcd.SetLineStyle(2);
    line_r_qcd.SetLineColor(kRed);
    line_r_qcd.Draw();
    
    c7.SaveAs("output_plots/QCD_CR_rapidity.png");
    ...
    leg8->SetFillStyle(0);
    leg8->AddEntry(&h_pt_data_QCD, "Data (2018)", "lep");
    leg8->AddEntry(&h_pt_qcd_QCD, "QCD (Data driven)", "f");
    leg8->AddEntry(&h_pt_dy_QCD, "Drell Yan", "f");
    leg8->AddEntry(&h_pt_ttjets_QCD, "t#bar{t}", "f");
    leg8->Draw();
    ...
    TLine line_pt_qcd(min_pt_QCD, 1.0, max_pt_QCD, 1.0);  // Use QCD ranges!
    line_pt_qcd.SetLineStyle(2);
    line_pt_qcd.SetLineColor(kRed);
    line_pt_qcd.Draw();
    
    c8.SaveAs("output_plots/QCD_CR_pt.png");
    ...
    leg9->SetFillStyle(0);
    leg9->AddEntry(&h_aco_data_TT, "Data (2018)", "lep");
    leg9->AddEntry(&h_aco_ttjets_TT, "t#bar{t}", "f");
    leg9->AddEntry(&h_aco_qcd_TT, "QCD (Data driven)", "f");
    leg9->AddEntry(&h_aco_dy_TT, "Drell Yan", "f");
    leg9->Draw();
    ...
    TLine line_aco_tt(min_aco_TT, 1.0, max_aco_TT, 1.0);  // Use TT ranges!
    line_aco_tt.SetLineStyle(2);
    line_aco_tt.SetLineColor(kRed);
    line_aco_tt.Draw();
    
    c9.SaveAs("output_plots/TT_CR_acoplanarity.png");
    ...
    leg10->AddEntry(&h_m_data_TT, "Data (2018)", "lep");
    leg10->AddEntry(&h_m_ttjets_TT, "t#bar{t}", "f");
    leg10->AddEntry(&h_m_qcd_TT, "QCD (Data driven)", "f");
    leg10->AddEntry(&h_m_dy_TT, "Drell Yan", "f");
    leg10->Draw();
    ...
    TLine line_m_tt(min_m_TT, 1.0, max_m_TT, 1.0);  // Use TT ranges!
    line_m_tt.SetLineStyle(2);
    line_m_tt.SetLineColor(kRed);
    line_m_tt.Draw();
    
    c10.SaveAs("output_plots/TT_CR_mass.png");
    ...
    leg11->AddEntry(&h_r_data_TT, "Data (2018)", "lep");
    leg11->AddEntry(&h_r_ttjets_TT, "t#bar{t}", "f");
    leg11->AddEntry(&h_r_qcd_TT, "QCD (Data driven)", "f");
    leg11->AddEntry(&h_r_dy_TT, "Drell Yan", "f");
    leg11->Draw();
    ...
    TLine line_r_tt(min_r_TT, 1.0, max_r_TT, 1.0);  // Use TT ranges!
    line_r_tt.SetLineStyle(2);
    line_r_tt.SetLineColor(kRed);
    line_r_tt.Draw();
    
    c11.SaveAs("output_plots/TT_CR_rapidity.png");
    ...
    leg12->AddEntry(&h_pt_data_TT, "Data (2018)", "lep");
    leg12->AddEntry(&h_pt_ttjets_TT, "t#bar{t}", "f");
    leg12->AddEntry(&h_pt_qcd_TT, "QCD (Data driven)", "f");
    leg12->AddEntry(&h_pt_dy_TT, "Drell Yan", "f");
    leg12->Draw();
    ...
    TLine line_pt_tt(min_pt_TT, 1.0, max_pt_TT, 1.0);  // Use TT ranges!
    line_pt_tt.SetLineStyle(2);
    line_pt_tt.SetLineColor(kRed);
    line_pt_tt.Draw();
    
    c12.SaveAs("output_plots/TT_CR_pt.png");

    cout << "All control region plots created successfully!" << endl;
    cout << "Generated 12 plots with FIXED ranges:" << endl;
    cout << "  DY Control Region: DY_CR_acoplanarity.png, DY_CR_mass.png, DY_CR_rapidity.png, DY_CR_pt.png" << endl;
    cout << "  QCD Control Region: QCD_CR_acoplanarity.png, QCD_CR_mass.png, QCD_CR_rapidity.png, QCD_CR_pt.png" << endl;
    cout << "  TT Control Region: TT_CR_acoplanarity.png, TT_CR_mass.png, TT_CR_rapidity.png, TT_CR_pt.png" << endl;
    cout << "Check the output_plots/ directory for all generated plots." << endl;


    cout << "========================================" << endl;
cout << "CONTROL REGION EVENT COUNTS:" << endl;
cout << "========================================" << endl;

cout << "DY CR:" << endl;
cout << "  Data events: " << h_aco_data_DY.GetEntries() << endl;
cout << "  DY MC events: " << h_aco_dy_DY.GetEntries() << endl;
cout << "  QCD MC events: " << h_aco_qcd_DY.GetEntries() << endl;
cout << "  ttjets MC events: " << h_aco_ttjets_DY.GetEntries() << endl;

cout << "QCD CR:" << endl;
cout << "  Data events: " << h_aco_data_QCD.GetEntries() << endl;
cout << "  DY MC events: " << h_aco_dy_QCD.GetEntries() << endl;
cout << "  QCD MC events: " << h_aco_qcd_QCD.GetEntries() << endl;
cout << "  ttjets MC events: " << h_aco_ttjets_QCD.GetEntries() << endl;

cout << "TT CR:" << endl;
cout << "  Data events: " << h_aco_data_TT.GetEntries() << endl;
cout << "  DY MC events: " << h_aco_dy_TT.GetEntries() << endl;
cout << "  QCD MC events: " << h_aco_qcd_TT.GetEntries() << endl;
cout << "  ttjets MC events: " << h_aco_ttjets_TT.GetEntries() << endl;
