...
int main(){
	...
	string prefix_output = "QCD_2018_UL_skimmed_MuTau_nano_";
...
out.Branch("generator_weight", &entry_30, "generator_weight/D");
	...
	for(int i=0; i<tree->GetEntries(); i++){
		...
		if( tree->GetLeaf("HLT_IsoMu24")->GetValue(0)==1){    //garante a presença de um muão
			...
			if ( tree->GetLeaf("Muon_pt")->GetLen()>0 && tree->GetLeaf("Tau_pt")->GetLen()>0 && tree->GetLeaf("Muon_mvaId")->GetValue(0)>=3 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSjet")->GetValue(0)>=63 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSe")->GetValue(0)>=7 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSmu")->GetValue(0)>=1 ){ //garante a presençã de um muão e de um tau hadronico
				nmutau++;
