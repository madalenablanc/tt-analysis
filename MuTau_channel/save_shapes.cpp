...
// Helper: load a BDT histogram from TMVA application outputs
TH1D* LoadBDTHist(const char* fileName, const char* newName, double scale = 1.0) {
    TFile input(fileName);
    if (input.IsZombie()) {
        cerr << "WARNING: cannot open BDT file " << fileName << endl;
        return nullptr;
    }
    TH1* hist = dynamic_cast<TH1*>(input.Get("MVA_BDT"));
    if (!hist) {
...
int main(){
    ...
    // ---- BDT histograms (20 bins from -0.8 to 0.8 generated by run_bdt_all.C) ----
    ...
    writeIfExists(bdt_data);
    writeIfExists(bdt_qcd);
    writeIfExists(bdt_dy);
    writeIfExists(bdt_ttjets);
    writeIfExists(bdt_sinal);
