...
void openFiles(){
 ...
 double entry_61, entry_62, entry_63, entry_64, entry_65, entry_66, entry_67, entry_68, entry_69, entry_73, entry_74, entry_76, entry_77, entry_78, entry_79;
...
xi_sist_2->Fit("xi_sist_inter_2");
 ...
 TFile fundo ("/eos/user/m/mpisano/analyzer_etau/CMSSW_12_5_0/src/ExclTauTau/ExclTauTau/test/Data_ETauTriggers_2018_UL_BSM_scalar_800_nov.root");
 TTree* ntp1 = (TTree*) fundo.Get("ntp1");
 TFile output ("./ETau_sinal_BSM_scalar_m800_nov.root", "RECREATE", "");
...
for (int i = 0; i < ntp1 -> GetEntries(); i++){
	...
	weight_sample = 54900.*1./(4000.*548.);
	...
	n_id= n_id + weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	
	if ((*tau_pt)[0] >100 && (*e_pt)[0] >35.0 ){
        n_pt = n_pt + weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;

	if (float((*e_charge)[0]) *float((*tau_charge)[0]) <0.0){
	n_charge = n_charge + weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	


	if (abs((*tau_eta)[0]) < 2.4 && abs((*e_eta)[0]) < 2.4) {
	n_eta = n_eta + weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	n_rec=n_rec+weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	//FIDUCIAL CUTS
		   ...
		   cout << "#######################ehi" << endl;
		    n_p_up_xi=n_p_up_xi+weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
		  ...
		  if(fraction >0.29 && fraction<=0.37) {
		   ...
		   //Era D2
		    ...
		    n_p_dw_xi=n_p_dw_xi+weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	n_p_rad=n_p_rad+weight*weight_sample*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	entry_65=weight_sample*weight*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	//entry_70=(*bsm_weights);
...
cout << "Os números apresentados têm em conta o semple weight" << endl;
