...
//	cout<<"ola";
	...
	string prefix_output = "QCD_CR_2018_UL_skimmed_ETau_nano_";
...
//		cout<<input<< " input"<<endl;
	...
	,entry_23,entry_24,entry_25,entry_26,entry_27,entry_28;
	int entry_29;
	//contadores
	int neletau=0;
	int nid=0;
...
//	cout<<"ola6"<<endl;
	...
	TTree *tree_lum = (TTree*) f->Get("LuminosityBlocks");
	////////////////////////////////////////////////////////////////////
...
out.Branch("met_phi", &entry_22, "met_phi/D");
 ...
 out.Branch("n_b_jet",&entry_29, "n_b_jet/I");
	...
	for(int i=0; i<tree->GetEntries(); i++){
		...
		int eventoslum=tree_lum->GetEvent(i);
...
//cout <<i<< "acontecimento aceite"<<endl;
//cout<<"//////////////////////"<<endl;


		if( tree->GetLeaf("HLT_Ele32_WPTight_Gsf")->GetValue(0)==1 && tree->GetLeaf("Electron_mvaFall17V2Iso_WP80")->GetValue(0)==1){

			if ( tree->GetLeaf("Electron_pt")->GetLen()>0 && tree->GetLeaf("Tau_pt")->GetLen()>0){
				neletau++;
				//if(tree->GetLeaf("MET_phi")->GetLen()>0 && tree->GetLeaf("Jet_phi")->GetLen()>0){
				//	double R=tau.DeltaR(electron);

				//if(tree->GetLeaf("Electron_mvaFall17V2Iso_WP80")->GetValue(0)==1 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSjet")->GetValue(0)<=64 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSe")->GetValue(0) <= 8 && tree->GetLeaf("Tau_idDeepTau2017v2p1VSmu")->GetValue(0) <= 2 && R>0.4){
				//	nid++;		
					
	
				//			if(tree->GetLeaf("Tau_charge")->GetValue(0)*tree->GetLeaf("Electron_charge")->GetValue(0)>0.){
				//				ncharge++;
				//			

				//				if(tree->GetLeaf("Electron_pt")->GetValue(0)>35. && tree->GetLeaf("Tau_pt")->GetValue(0)>100.){
				//					npt++;
								
									
				//					if(fabs(tree->GetLeaf("Tau_eta")->GetValue(0))<2.4 && tree->GetLeaf("Electron_eta")->GetValue(0)<2.4){
