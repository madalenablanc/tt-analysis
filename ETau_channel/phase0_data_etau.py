...
# If your site has IPv6 quirks, uncomment the next line
# os.environ.setdefault("XRD_NETWORKSTACK", "IPv4")
# XRootD timeouts (tuned to be resilient but not too slow)
...
# We only enable MT in driver; worker will disable to avoid rare segfaults
ROOT.EnableImplicitMT()

# ----------------------------- Configuration ------------------------------
line_number   = -1   # -1 => process all lines; >=0 => only that index
input_list    = "Data_ETau_phase0_2018.txt"      # one /store/... path (or full PFN) per line
output_prefix = "/eos/user/m/mblancco/samples_2018_etau/fase0_etau/Data_2018_UL_skimmed_ETau_nano_"
lumi_file     = "dadosluminosidade.txt"
resume_path   = ".etau_phase0_resume.json"
overwrite     = False
proxy_min_left_sec = 30*60             # auto-renew proxy if < 30 min left
CACHE_DIR     = os.path.expanduser("~/.etau_cache")
...
def xrdcp_with_retries(pfn, dst, tries=3, sleep=5):
    for k in range(tries):
        try:
            ret = subprocess.run(["xrdcp", "-f", "-s", "--cksum", "adler32", pfn, dst],
...
# ----------------------------- Snapshot columns ---------------------------
COLUMNS = [
    "electron_id","tau_id1","tau_id2","tau_id3",
    "electron_pt","tau_pt",
    "electron_charge","tau_charge",
    "electron_eta","tau_eta",
    "electron_n","tau_n",
...
def process_index(idx):
    ...
    # Worker is single-threaded for stability (remote I/O)
    ...
    def try_rdf(input_spec):
        try:
            df = ROOT.RDataFrame("Events", input_spec)
            df_sel = (
                df.Filter("lumi_filter(run, luminosityBlock)", "Certified lumi")
                  .Filter("HLT_Ele32_WPTight_Gsf == 1", "HLT Electron trigger")
                  .Filter("Electron_pt.size() > 0 && Tau_pt.size() > 0", "Electron and tau present")
                  .Filter("Electron_mvaFall17V2Iso_WP80[0] == 1", "Electron ID WP80")
                  ...
                  .Filter("Electron_pt[0] > 35. && Tau_pt[0] > 100.", "pT thresholds")
                  .Filter("Electron_charge[0] * Tau_charge[0] < 0", "Opposite sign")
            )
            df_defs = (
                df_sel
                .Define("electron_pt",   "Electron_pt[0]")
                ...
                .Define("electron_charge","Electron_charge[0]")
                .Define("tau_charge",    "Tau_charge[0]")
                .Define("electron_id",   "Electron_mvaFall17V2Iso_WP80[0]")
                ...
                .Define("electron_n",    "int(Electron_pt.size())")
                .Define("tau_n",         "int(Tau_pt.size())")
                .Define("electron", "TLorentzVector el; el.SetPtEtaPhiM(electron_pt,electron_eta,electron_phi,electron_mass); return el;")
                .Define("tau",      "TLorentzVector t;  t.SetPtEtaPhiM(tau_pt,tau_eta,tau_phi,tau_mass); return t;")
                .Define("sistema",  "return electron + tau;")
                .Define("sist_mass","sistema.M()")
                .Define("sist_pt",  "sistema.Pt()")
                .Define("sist_rap", "sistema.Rapidity()")
                .Define("acop",     "double dphi=fabs(electron_phi-tau_phi); if(dphi>M_PI) dphi=2*M_PI-dphi; return fabs(dphi)/M_PI;")
            ...
            print(f"⚠️ RDF failed on {input_spec}: {e}")
            return False

    # A) Try remote PFN
    if try_rdf(pfn):
        print(f"✅ finished remote {out_file}")
        return 0

    # B) Fall back to local cache (xrdcp) and retry
    ...
    print("⛔ failed both remote and local processing")
    return 4

# ------------------------------- Driver mode -------------------------------
def run_worker(single_idx):
    # Run worker in a subprocess (isolation from crashes)
    env = os.environ.copy()
...
def main():
    ...
    print("✅ ETau phase-0 data skimming completed for requested files.")
